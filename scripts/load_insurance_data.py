...
def load_and_convert():
    ...
    # Client columns  
    client_cols = [col for col in df.columns if col in ['IsVATRegistered', 'Citizenship', 'LegalType', 
                                                       'Title', 'Language', 'Bank', 'AccountType', 
                                                       'MaritalStatus', 'Gender']]
    print(f"  • Client columns: {len(client_cols)}")
    
    # Location columns
    location_cols = [col for col in df.columns if col in ['Country', 'Province', 'PostalCode', 
                                                         'MainCrestaZone', 'SubCrestaZone']]
    print(f"  • Location columns: {len(location_cols)}")
    
    # Vehicle columns
    vehicle_cols = [col for col in df.columns if col in ['ItemType', 'mmcode', 'VehicleType', 
                                                        'RegistrationYear', 'make', 'Model', 
                                                        ...
                                                        'NewVehicle', 'WrittenOff', 'Rebuilt', 
                                                        'Converted', 'CrossBorder', 'NumberOfVehiclesInFleet']]
    print(f"  • Vehicle columns: {len(vehicle_cols)}")
    
    # Plan columns
    plan_cols = [col for col in df.columns if col in ['SumInsured', 'TermFrequency', 'CalculatedPremiumPerTerm', 
